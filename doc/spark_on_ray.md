...
+ Spark and Ray may use different log4j versions. For example, Spark 3.2 or older uses log4j 1 and Ray uses log4j 2 for long time. And they use different log4j configuration files.
+ To use your customized log4j versions for Spark and ray, you can set `spark.preferClassPath` and `spark.ray.preferClassPath` respectively to include your log4j jars when `init_spark` as long as your log4j versions are compatible with Spark and Ray respectively.
  ...
  ```
+ For log4j config files, you can set `spark.log4j.config.file.name` and `spark.ray.log4j.config.file.name` for Spark and Ray respectively.
