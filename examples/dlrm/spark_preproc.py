...
class PreProcArguments:
    def __init__(self,
                 ...
                 skew_broadcast_pct=1.0,
                 debug_mode=False):

        start, end = total_days.split("-")
        self.total_days_range = list(range(int(start), int(end) + 1))
        start, end = train_days.split('-')
        self.train_days_range = list(range(int(start), int(end) + 1))

        if test_days is not None:
            start, end = test_days.split('-')
            self.test_days_range = list(range(int(start), int(end) + 1))
        else:
        ...
        self.no_numeric_log_col = no_numeric_log_col
        self.low_mem = low_mem
        self.output_ordering = output_ordering
        self.output_partitioning = output_partitioning
        self.dict_build_shuffle_parallel_per_day = dict_build_shuffle_parallel_per_day
        self.apply_shuffle_parallel_per_day = apply_shuffle_parallel_per_day
        self.skew_broadcast_pct = skew_broadcast_pct
...
def transform(spark, args: PreProcArguments, day_range, is_train: bool, return_df: bool):
    ...
    with _timed('transform'):
        ...
        if args.model_size_file:
            model_size = OrderedDict(('_c%d' % i, agg.size) for i, _, agg, _ in models)
        ...
        if args.output_ordering == 'total_random':
            if args.output_partitioning == 'day':
                df = psudo_sort_by_day_plus(spark, df, days)
            else:  # none
