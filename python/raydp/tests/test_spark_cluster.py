...
def test_placement_group(ray_cluster):
    for pg_strategy in ["PACK", "STRICT_PACK", "SPREAD", "STRICT_SPREAD"]:
        spark = raydp.init_spark(f"test_strategy_{pg_strategy}_1", 1, 1, "500 M",
        ...
        ray.get(pg.ready())
        spark = raydp.init_spark(f"test_bundle_{pg_strategy}_2", 1, 1, "500 M",
                                 placement_group=pg,
        ...
        spark = raydp.init_spark(f"test_bundle_{pg_strategy}_3", 1, 1, "500 M",
                                 placement_group=pg)
        result = spark.range(0, 10, numPartitions=10).count()
        assert result == 10
        raydp.stop_spark()
